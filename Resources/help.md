...
## Verbatim mode
Smart Cleanup (Settings → Mode) adjusts punctuation and capitalization while preserving every dictated word. Semicolons are rare in the user's style, so cleanup does not introduce one unless it was already present in the raw transcription. It also does not create a new sentence beginning with “And” unless that was already present in the raw transcription. If the cleanup model adds, removes, or repeats wording, Vox discards that cleanup and keeps the pre-cleanup transcription. When you pause after an unfinished phrase and say `scratch that, …`, Vox replaces only that unfinished phrase back to the nearest clause boundary while keeping the sentence context; this works when the pause is transcribed as a comma, dash, or ellipsis. Personalization → **Custom Instructions** can use the inline `cleanup-profile.md` fallback or a linked Markdown file that is read fresh and sent to the configured OpenAI cleanup provider with each eligible dictation. Sometimes you want the literal text without cleanup. Two ways to bypass cleanup for a single recording:
