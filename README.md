...
- **Smart cleanup** — opt-in LLM cleanup via gpt-4o-mini removes obvious false starts, fillers, and self-corrections in prose. Cleanup is deletion-only at the word level: if the model introduces any word that was not dictated, Vox discards the cleanup and uses the pre-cleanup transcription, preventing questions or requests from becoming assistant answers. Personalization → **Custom Instructions** can use the inline `cleanup-profile.md` fallback or a linked Markdown file that is read fresh and sent to the configured OpenAI cleanup provider with each eligible dictation. Bypassed by verbatim modifier or "verbatim"/"literal" prefix word.
